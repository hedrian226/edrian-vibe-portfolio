...
const projects = [
  ...
  { number:'04', type:'WEB APP · LIVE', title:'Edsxel Ngani', description:'A friendly Excel-formulas hub — plain-English tutorials, interactive practice tools, a formula decoder and guided learning paths for spreadsheet skills.', tags:['Excel Tools','Tutorials','Vercel'], url:'https://edsxel-ngani.vercel.app/', preview:'excel' },
];
        ...
        <div className="project-list">{projects.map(project=><TiltCard key={project.number} className="project-card"><a href={project.url} target="_blank" rel="noopener noreferrer" className="project-inner" data-reveal data-cursor="VIEW ↗">
          ...
          <div className="project-body"><div><h3>{project.title}</h3><p>{project.description}</p><div className="tag-row">{project.tags.map(tag=><span key={tag}>{tag}</span>)}</div></div>
            <div className={`project-preview ${project.preview==='rollcall'?'roll-call-preview':project.preview==='maxcare'?'maxcare-preview':project.preview==='brew'?'brew-preview':'excel-preview'}`}>
              <div className="preview-top"><i/><i/><i/><span>{project.preview==='rollcall'?'roll-call-app':project.preview==='maxcare'?'maxcare-shop':project.preview==='brew'?'brew-dxb':'edsxel-ngani'}</span></div>
              <div className="preview-content">
                ...
                {project.preview==='excel' && <div className="excel-screen"><div className="excel-hud"><span className="excel-logo">EDSXEL <b>NGANI</b></span><span className="excel-lesson">LESSON 04/12</span></div><div className="excel-cards"><span><i/><b>VLOOKUP</b><small>LOOKUP FORMULA</small></span><span><i/><b>SUMIFS</b><small>CONDITIONAL SUM</small></span><span><i/><b>INDEX/MATCH</b><small>FLEXIBLE LOOKUP</small></span></div><div className="excel-tray"><b>fx</b><span>FORMULA DECODED</span><i>KEEP LEARNING ↗</i></div></div>}
              </div>
            </div>
          </div>
        </a></TiltCard>)}</div>

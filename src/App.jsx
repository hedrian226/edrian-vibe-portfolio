...
const projects = [
  ...
  { number:'02', type:'COMMERCE · LIVE', title:'MaxCare Shop', description:'A shoppable beauty-supply catalogue for the UAE — product search, category filters, multi-item enquiry building and direct WhatsApp ordering.', tags:['Product Catalog','Enquiry Flow','Vercel'], url:'https://maxcare-shop.vercel.app/', featured:false },
];
      ...
      <section id="work" className="section work-section">
        ...
        <div className="project-list">{projects.map(project=><TiltCard key={project.number} className="project-card"><a href={project.url} target="_blank" rel="noopener noreferrer" className="project-inner" data-reveal data-cursor="VIEW ↗">
          ...
          <div className="project-body"><div><h3>{project.title}</h3><p>{project.description}</p><div className="tag-row">{project.tags.map(tag=><span key={tag}>{tag}</span>)}</div></div>
            <div className={`project-preview ${project.featured?'roll-call-preview':'maxcare-preview'}`}><div className="preview-top"><i/><i/><i/><span>{project.featured?'roll-call-app':'maxcare-shop'}</span></div><div className="preview-content">{project.featured?<><div className="preview-title">TODAY'S ROLL CALL</div><div className="preview-stat"><AnimatedNumber value={24}/><span>employees</span></div><div className="preview-checkins"><span><i/>E. SANTOS <b>IN</b></span><span><i/>M. CRUZ <b>IN</b></span><span><i/>A. REYES <b>IN</b></span></div><div className="preview-bars"><i style={{'--bar':'72%'}}/><i style={{'--bar':'48%'}}/><i style={{'--bar':'86%'}}/><i style={{'--bar':'58%'}}/></div><div className="preview-toast"><i>✓</i><span>ATTENDANCE SYNCED</span></div></>:<div className="maxcare-screen"><div className="maxcare-nav"><b>max<span>Care</span></b><small>SHOP · WHY · CONTACT</small><i>WHATSAPP</i></div><div className="maxcare-hero"><div><small>DIRECT SALON SUPPLIER · UAE</small><strong>Beauty stock,<br/><em>ready to order.</em></strong><span className="maxcare-button">BROWSE PRODUCTS</span></div><div className="maxcare-products"><span><i/><b>LIGUI</b><small>HAIR COLOUR</small></span><span><i/><b>AIRETTE</b><small>TREATMENT</small></span><span><i/><b>GZQM</b><small>HAIR CARE</small></span></div></div><div className="maxcare-enquiry"><b>02</b><span>PRODUCTS SAVED</span><i>VIEW ENQUIRY ↗</i></div></div>}</div></div>
          </div>
        </a></TiltCard>)}</div>

...
const projects = [
  ...
  { number:'04', type:'WEB APP · LIVE', title:'Formula Lab', description:'A friendly Excel-formulas hub — plain-English tutorials, interactive practice tools, a formula decoder and guided learning paths for spreadsheet skills.', tags:['Excel Tools','Tutorials','Vercel'], url:'https://formula-lab-nine.vercel.app/', preview:'excel' },
  { number:'05', type:'CLINIC SITE · LIVE', title:'Encita Lying-in Clinic', description:'A maternity clinic site for General Trias, Cavite — services and PhilHealth coverage, patient recommendations pulled from Facebook, and clinic location and contact details.', tags:['Healthcare','React','Vercel'], url:'https://encita-lying-in-clinic.vercel.app/', preview:'clinic' },
];
      ...
      <section id="work" className="section work-section">
        ...
        <div className="project-list">{projects.map(project=><TiltCard key={project.number} className="project-card"><a href={project.url} target="_blank" rel="noopener noreferrer" className="project-inner" data-reveal data-cursor="VIEW ↗">
          ...
          <div className="project-body"><div><h3>{project.title}</h3><p>{project.description}</p><div className="tag-row">{project.tags.map(tag=><span key={tag}>{tag}</span>)}</div></div>
            <div className={`project-preview ${project.preview==='rollcall'?'roll-call-preview':project.preview==='maxcare'?'maxcare-preview':project.preview==='brew'?'brew-preview':project.preview==='excel'?'excel-preview':'clinic-preview'}`}>
              <div className="preview-top"><i/><i/><i/><span>{project.preview==='rollcall'?'roll-call-app':project.preview==='maxcare'?'maxcare-shop':project.preview==='brew'?'brew-dxb':project.preview==='excel'?'formula-lab':'encita-clinic'}</span></div>
              <div className="preview-content">
                ...
                {project.preview==='excel' && <div className="excel-screen"><div className="excel-hud"><span className="excel-logo">FORMULA <b>LAB</b></span><span className="excel-lesson">LESSON 04/12</span></div><div className="excel-cards"><span><i/><b>VLOOKUP</b><small>LOOKUP FORMULA</small></span><span><i/><b>SUMIFS</b><small>CONDITIONAL SUM</small></span><span><i/><b>INDEX/MATCH</b><small>FLEXIBLE LOOKUP</small></span></div><div className="excel-tray"><b>fx</b><span>FORMULA DECODED</span><i>KEEP LEARNING ↗</i></div></div>}
                {project.preview==='clinic' && <div className="clinic-screen"><div className="clinic-hud"><span className="clinic-logo">ENCITA <b>CLINIC</b></span><span className="clinic-tagline">GENERAL TRIAS · CAVITE</span></div><div className="clinic-cards"><span><i/><b>PRENATAL</b><small>OB-GYN CHECKUPS</small></span><span><i/><b>DELIVERY</b><small>MIDWIFE-ATTENDED</small></span><span><i/><b>PEDIATRIC</b><small>WELL-BABY CARE</small></span></div><div className="clinic-tray"><b>♥</b><span>100% RECOMMENDED</span><i>14 REVIEWS ↗</i></div></div>}
              </div>
            </div>
          </div>
        </a></TiltCard>)}</div>
